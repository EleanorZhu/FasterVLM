...
class TextVQAPromptTuningDataset(Dataset):
    """
    Dataset for training prompt tuning on TextVQA (FIXED VERSION).

    FIXED: Separates question and answer inputs to ensure train/inference consistency.
    - question_input_ids: Only contains question (for intermediate extraction)
    - answer_input_ids: Only contains answer (appended after intermediate extraction)
    ...
    def __getitem__(self, idx):
        ...
        # FIXED: Separate question and answer inputs
        # 1. Question input (for intermediate extraction) - NO answer
        conv_question = conversation_lib.conv_templates[self.conv_mode].copy()
        conv_question.append_message(conv_question.roles[0], DEFAULT_IMAGE_TOKEN + "\n" + question_text)
        conv_question.append_message(conv_question.roles[1], None)  # No answer!
        question_prompt = conv_question.get_prompt()

        # Tokenize question
        question_input_ids = tokenizer_image_token(question_prompt, self.tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt')

        # 2. Answer input (to append after intermediate extraction)
        # Get the separator to construct answer text
        conv_temp = conversation_lib.conv_templates[self.conv_mode].copy()
        answer_text = answer + conv_temp.sep  # e.g., "DAKOTA</s>"

        # Tokenize answer (without special tokens at the beginning)
        ...
        # 3. Create labels
        # Labels should cover: [question (masked)] + [answer (unmasked)]
        # But we don't concatenate them here - the model will do it
        # We just need to know which tokens to compute loss on

        # For the model's forward pass:
        # - input_ids = question only
        # - answer_input_ids = answer only
        # - labels = will be constructed to mask question and unmask answer

        # Create labels: question part is IGNORE_INDEX, answer part is the actual tokens
        question_labels = torch.full_like(question_input_ids, IGNORE_INDEX)
        answer_labels = answer_input_ids.clone()

        # Note: The model will concatenate these, so we prepare them separately
        # labels will be: [IGNORE, IGNORE, ..., IGNORE, answer_token_1, answer_token_2, ...]

        return {
            'input_ids': question_input_ids,  # Question only (for intermediate extraction)
            'answer_input_ids': answer_input_ids,  # Answer only (to append after)
            'question_labels': question_labels,  # For question part (all IGNORE)
            'answer_labels': answer_labels,  # For answer part (actual tokens)
            'image': image_tensor,
...
def collate_fn(batch):
    """Collate function for DataLoader (FIXED VERSION)."""
    # Find max lengths for question and answer separately
    max_question_len = max(item['input_ids'].shape[0] for item in batch)
    max_answer_len = max(item['answer_input_ids'].shape[0] for item in batch)
    ...
    for item in batch:
        # Pad question input_ids
        question_input_ids = item['input_ids']
        question_padding_len = max_question_len - question_input_ids.shape[0]
        if question_padding_len > 0:
            question_input_ids = torch.cat([
                question_input_ids,
                torch.full((question_padding_len,), 0, dtype=question_input_ids.dtype)
            ])

        # Pad answer input_ids
        answer_input_ids = item['answer_input_ids']
        answer_padding_len = max_answer_len - answer_input_ids.shape[0]
        if answer_padding_len > 0:
            answer_input_ids = torch.cat([
                answer_input_ids,
                torch.full((answer_padding_len,), 0, dtype=answer_input_ids.dtype)
            ])

        # Pad question labels
        question_labels = item['question_labels']
        if question_padding_len > 0:
            question_labels = torch.cat([
                question_labels,
                torch.full((question_padding_len,), IGNORE_INDEX, dtype=question_labels.dtype)
            ])
        ...
        if answer_padding_len > 0:
            answer_labels = torch.cat([
                answer_labels,
                torch.full((answer_padding_len,), IGNORE_INDEX, dtype=answer_labels.dtype)
            ])

        input_ids_list.append(question_input_ids)
        answer_input_ids_list.append(answer_input_ids)
        question_labels_list.append(question_labels)
    ...
    answer_input_ids_batch = torch.stack(answer_input_ids_list)
    question_labels_batch = torch.stack(question_labels_list)
    answer_labels_batch = torch.stack(answer_labels_list)

    # FIXED: Don't concatenate labels here - let the model do it
    # The model will concatenate labels after adding answer_input_ids
    # For now, labels should only correspond to the question part
    labels_batch = question_labels_batch

    return {
        'input_ids': input_ids_batch,  # Question only
        'answer_input_ids': answer_input_ids_batch,  # Answer only
        'labels': labels_batch,  # Question labels only (model will extend with answer labels)
        'answer_labels': answer_labels_batch,  # Answer labels (for model to append)
        'images': torch.stack(images),
        'attention_mask': input_ids_batch.ne(0),  # Attention mask for question
...
def convert_to_intermediate_layer_model(
    ...
    """
    Convert a base LLaVA model to use intermediate layer feedback with prompt tuning (FIXED VERSION).

    This function uses object.__new__() to create a new instance without calling __init__,
    then transfers all attributes from base_model to avoid duplicating memory.
    """
    # Create a new instance WITHOUT calling __init__ (to avoid reinitializing weights)
    new_model = object.__new__(LlavaIntermediateLayerForCausalLMFixed)

    # Transfer ALL attributes from base_model to new_model
    new_model.__dict__.update(base_model.__dict__)

    # Update config with intermediate layer settings
    new_model.config.intermediate_layer_idx = intermediate_layer_idx
    new_model.config.use_intermediate_feedback = True
    new_model.config.use_prompt_tuning = True
    new_model.config.num_prompt_tokens = num_prompt_tokens
    new_model.config.prompt_init_method = prompt_init_method
    new_model.config.model_type = "llava_intermediate_layer_fixed"

    # Store visual_token_num
    new_model.visual_token_num = visual_token_num

    # Set the new attributes that LlavaIntermediateLayerForCausalLMFixed needs
    new_model.intermediate_layer_idx = intermediate_layer_idx
    new_model.use_intermediate_feedback = True
    new_model.use_prompt_tuning = True
    new_model.num_prompt_tokens = num_prompt_tokens
    new_model.prompt_init_method = prompt_init_method

    # Initialize prompt embeddings
    hidden_size = new_model.config.hidden_size
    new_model.prompt_embeddings = torch.nn.Parameter(
        torch.zeros(num_prompt_tokens, hidden_size)
    )

    # Initialize prompt embeddings based on method
    if prompt_init_method == "random":
        torch.nn.init.normal_(new_model.prompt_embeddings, mean=0.0, std=0.02)
    elif prompt_init_method == "from_vocab":
        # Sample from vocabulary embeddings
        embed_tokens = new_model.get_model().embed_tokens
        vocab_size = embed_tokens.weight.shape[0]
        random_indices = torch.randint(0, vocab_size, (num_prompt_tokens,))
        with torch.no_grad():
            new_model.prompt_embeddings.data = embed_tokens.weight[random_indices].clone()

    print(f"[PromptTuning] Initialized {num_prompt_tokens} prompt tokens with method '{prompt_init_method}'")
    print(f"[IntermediateLayer] Initialized with layer_idx={intermediate_layer_idx}, feedback=True, prompt_tuning=True")

    # Return the new model (shares all weights with base_model except prompt_embeddings)
    return new_model
...
def train(args):
    ...
    # Convert to intermediate layer model with prompt tuning (in-place conversion)
    print(f"Converting to intermediate layer model with prompt tuning (in-place)...")
    model = convert_to_intermediate_layer_model(
        model,  # Pass the model itself, will be modified in-place
        tokenizer,
        image_processor,
    ...
    # Move vision tower to GPU with fp16
    ...
    # Determine actual vision tower dtype and align projector
    try:
    ...
    if hasattr(model, 'prompt_embeddings'):
        ...
        print(f"[PromptTuning] Converted prompt_embeddings to FP32 for stable training")
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch in enumerate(train_bar):
            input_ids = batch['input_ids'].to(device)  # Question only
            answer_input_ids = batch['answer_input_ids'].to(device)  # Answer only
            answer_labels = batch['answer_labels'].to(device)  # Answer labels (NEW)
            labels = batch['labels'].to(device)  # Question labels
            ...
            with autocast(dtype=torch.float16):
                outputs = model(
                    input_ids=input_ids,  # Question only (for intermediate extraction)
                    answer_input_ids=answer_input_ids,  # Answer (appended after intermediate extraction)
                    answer_labels=answer_labels,  # Answer labels (NEW)
                    attention_mask=attention_mask,
                    labels=labels,  # Question labels
                    images=images,
                    use_cache=False
                )
                loss = outputs.loss
            ...
            # Skip batches with non-finite loss or no valid answer tokens
            # Note: We check answer_labels instead of labels because labels only contains question (all IGNORE)
            valid_answer_tokens = (answer_labels != IGNORE_INDEX).sum().item()
            if valid_answer_tokens == 0 or not torch.isfinite(loss):
        ...
        model.eval()
        val_loss = 0.0
        val_count = 0  # Count valid batches
        ...
        with torch.no_grad():
            for batch in val_bar:
                ...
                with autocast(dtype=torch.float16):
                    outputs = model(
                        input_ids=input_ids,  # Question only
                ...
                if torch.isfinite(loss):
                    ...
                    # Debug: print why loss is NaN
                    valid_answer_tokens = (answer_labels != IGNORE_INDEX).sum().item()
                    if val_count == 0:  # Only print once
                        print(f"\n[DEBUG] NaN loss detected!")
                        print(f"  - Valid answer tokens: {valid_answer_tokens}")
                        print(f"  - answer_labels shape: {answer_labels.shape}")
                        print(f"  - answer_labels sample: {answer_labels[0][:20]}")
                    val_bar.set_postfix({'loss': 'nan (skipped)'})
        ...
        if avg_val_loss < best_val_loss:
            ...
            output_path = os.path.join(
                args.output_dir,
                f"prompt_tokens_layer{args.intermediate_layer_idx}_n{args.num_prompt_tokens}_fixed.pt"  # Add "_fixed" suffix
            )
            torch.save({
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-prompt-tokens", type=int, default=20, help="Number of learnable prompt tokens")
